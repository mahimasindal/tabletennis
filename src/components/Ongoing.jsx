...
function Ongoing(){
    var [tournaments, setItems]=useState([]);
    var [isLoaded, setLoaded]=useState(false);

    useEffect(()=>{
        const token='Bearer '+localStorage.getItem('token');
        const URL="http://139.59.16.180:8989/tournament/list/ongoing";
        ...
        .then(res => {
            ...
            setItems(res.data)
            setLoaded(true)
            console.log(isLoaded===true, isLoaded)
        })
        .catch(err=>{
            console.log(err)
   ...
   if(isLoaded===true && tournaments===[])
   return(<div>No Ongoing Tournaments</div>)
    return(
            <div className="tournamentList">
            <div>{isLoaded===true && tournaments===[]}</div>
